...
def year_match_reader(year):
    df = pd.read_csv('https://raw.githubusercontent.com/JeffSackmann/tennis_wta/master/wta_matches_'+ str(year) + '.csv', encoding = 'latin-1', quoting=csv.QUOTE_NONE)
    df['year'] = year
    return df

wta_matches = pd.concat([year_match_reader(y) for y in range(2020, 2024)])
wta_matches['tourney_date'] = pd.to_datetime(wta_matches['tourney_date'], format='%Y%m%d')

### Score cleaner
def score_cleaner(score, player_is_winner=True):
    return int(score.split('(')[0].split('-')[1-player_is_winner])

wta_matches['score'] = [s.split('[')[0].strip() for s in wta_matches['score']]
wta_matches = wta_matches.loc[(['RET' not in s for s in wta_matches['score']])]
wta_matches = wta_matches.loc[(['W/O' not in s for s in wta_matches['score']])]
wta_matches = wta_matches.loc[(['Def.' not in s for s in wta_matches['score']])]
wta_matches['winner_games_won'] = wta_matches['score'].apply(lambda x: np.sum([score_cleaner(score = s, player_is_winner=True) for s in x.split(' ') if s not in ('RET', 'W/O', 'Def.')]))
wta_matches['loser_games_won'] = wta_matches['score'].apply(lambda x: np.sum([score_cleaner(score = s, player_is_winner=False) for s in x.split(' ') if s not in ('RET', 'W/O', 'Def.')]))
wta_matches['games_played'] = wta_matches['winner_games_won'] + wta_matches['loser_games_won']
wta_matches['GmWinPercWinner'] = wta_matches['winner_games_won']/(wta_matches['winner_games_won'] + wta_matches['loser_games_won'])
wta_matches['GmWinPercLoser'] = wta_matches['loser_games_won']/(wta_matches['winner_games_won'] + wta_matches['loser_games_won'])
wta_matches.loc[(wta_matches['games_played']>0)]

wta_matches['round_num'] = 0
wta_matches['round_num'].loc[wta_matches['round']=='R128'] = 1
wta_matches['round_num'].loc[wta_matches['round']=='R64'] = 2
wta_matches['round_num'].loc[wta_matches['round']=='R32'] = 3
wta_matches['round_num'].loc[wta_matches['round']=='R16'] = 4
wta_matches['round_num'].loc[wta_matches['round']=='QF'] = 5
wta_matches['round_num'].loc[wta_matches['round']=='SF'] = 6
wta_matches['round_num'].loc[wta_matches['round']=='F'] = 7

wta_matches = wta_matches.merge(wta_matches[['tourney_id', 'round_num']].groupby('tourney_id', as_index=False).min(), on = 'tourney_id', suffixes=['','_min'])
wta_matches['match_date'] = wta_matches.apply(lambda x: x['tourney_date'] + timedelta(days=2*(x['round_num'] - x['round_num_min'])), axis = 1)
wta_matches['match_id'] = wta_matches.apply(lambda x: x['match_date'].strftime("%Y%m%d") + '_' + x['winner_name']+ '_' + x['loser_name'], axis=1)
wta_matches['surface'] = wta_matches['surface'].str.lower()
wta_matches['surface'] = wta_matches['surface'].fillna('hard')
wta_matches = wta_matches.sort_values('match_date')
...
players_elo_df = pd.DataFrame({
        'player': np.sort(np.unique(wta_matches['winner_name'].unique().tolist() + wta_matches['loser_name'].unique().tolist())),
        'ds': wta_matches['match_date'].min()-timedelta(days=1),
        'outcome': None,
        'opponent': None,
        'surface': None,
...
def process_match(match_id, ds, player_1, player_2, surface):
    tmp_match = wta_matches.loc[wta_matches['match_id']==match_id]
...
for i in range(wta_matches.shape[0]):
    players_elo_df = pd.concat([
            players_elo_df,
            process_match(match_id=wta_matches['match_id'].iloc[i], ds=wta_matches['match_date'].iloc[i], player_1=wta_matches['winner_name'].iloc[i], player_2=wta_matches['loser_name'].iloc[i], surface=wta_matches['surface'].iloc[i])
        ]).drop_duplicates(['player', 'ds'])
    
    print('[' + str(round(100*i/wta_matches.shape[0], 2)) + '%] ||| ||| ' + wta_matches['match_id'].iloc[i])
...
### Save out
players_elo_df.to_csv('~/Documents/Personal Code/Project_Baseline/women/csv/players_elo_df.csv', index=False)

##################
### Neural Net ###
##################

players_elo_df = pd.read_csv('~/Documents/Personal Code/Project_Baseline/women/csv/players_elo_df.csv')
...
######################################
currrent_tourn = pd.read_csv('~/Documents/Personal Code/Project_Baseline/women/csv/current_tournament_draw.csv')
currrent_tourn['player'] = currrent_tourn['player'].apply(lambda x: x.replace('_',''))
currrent_tourn.to_csv('~/Documents/Personal Code/Project_Baseline/women/csv/current_tournament_draw.csv', index=False)
...
### Save
matchups[['player', 'player_opponent', 'win_prob_mean']].to_csv('~/Documents/Personal Code/Project_Baseline/women/csv/current_tourn_matchups_win_prob.csv', index=False)
